...
import { DATA_NAJDI_SLOVESA } from "@/src/data/najdi-slovesa";
...
type Sentence = (typeof DATA_NAJDI_SLOVESA)[number];
...
function pickSentences(): Sentence[] {
  return shuffleArray(DATA_NAJDI_SLOVESA).slice(0, NUM_SENTENCES);
}
...
export default function NajdiSlovesaPage() {
  ...
  const pct = totalVerbs > 0 ? Math.round((score / totalVerbs) * 100) : 0;
  ...
  const hasErrors = useMemo(
    () =>
      results.some(
        (result) =>
          !result.verbs.every((idx) => result.clicked.includes(idx)) ||
          result.clicked.some((idx) => !result.verbs.includes(idx)),
      ),
    [results],
  );
  ...
  const setUpRound = (index: number, nextSentences: Sentence[], baseResults: SentenceResult[]) => {
    ...
    nextResults[index] = { words: sentence.words, verbs: sentence.verbs, clicked: [] };

    setCurrentIndex(index);
    setClicksLeft(sentence.verbs.length);
  ...
  const restart = () => {
    ...
    const nextTotalVerbs = nextSentences.reduce((sum, sentence) => sum + sentence.verbs.length, 0);

    setSentences(nextSentences);
    setScore(0);
    setTotalVerbs(nextTotalVerbs);
  ...
  const finishSentence = (result: SentenceResult, baseWordStates: WordState[]) => {
    ...
    currentSentence.verbs.forEach((idx) => {
    ...
    const allCorrect = currentSentence.verbs.every((idx) => result.clicked.includes(idx));
    if (allCorrect) {
  ...
  const handleWordClick = (index: number) => {
    ...
    if (currentSentence.verbs.includes(index)) {
      ...
      setFeedback("Skús ešte raz, toto nie je sloveso.");
      setFeedbackTone("bad");
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-100 to-teal-50 font-sans text-slate-800">
      {!showResults ? (
        <div>
            ...
            {sentences.map((_, i) => (
              <div
                ...
                className={cn(
                  ...
                  i === currentIndex && "scale-125 bg-teal-500",
                  i > currentIndex && "bg-teal-200/80",
                )}
              />
            ))}
          </div>
          ...
          <main className="mx-auto flex min-h-[calc(100vh-7rem)] w-full max-w-5xl flex-col items-center justify-center px-4 py-8 sm:px-6">
            <p className="mb-2 rounded-xl bg-white/70 px-4 py-2 text-center text-lg font-semibold text-slate-500 sm:text-xl">
              Klikni na slová, ktoré sú slovesá
            </p>
            <p className="mb-7 text-center text-base font-medium text-teal-700">Každá správna voľba je bod navyše.</p>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      state === "idle" &&
                        "border-teal-300 hover:-translate-y-0.5 hover:border-teal-400 hover:bg-teal-50",
            ...
            <div
              className={cn(
                ...
                feedbackTone === "complete" && "text-teal-600",
              )}
            >
              {feedback}
            </div>
            ...
            {showNext && (
              <button
                type="button"
                className="mt-3 min-h-14 rounded-2xl bg-teal-500 px-10 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-teal-600"
        ...
        <section className="mx-auto w-full max-w-4xl px-4 py-12 text-center">
          <h2 className="text-4xl font-black text-teal-600">Hotovo!</h2>
          ...
          <div className="mx-auto mt-8 max-w-3xl text-left">
            ...
            {results.map((result, resultIdx) => {
              const isAllCorrect =
                result.verbs.every((idx) => result.clicked.includes(idx)) &&
                result.clicked.every((idx) => result.verbs.includes(idx));
              ...
              return (
                ...
                >
                  {result.words.map((word, wordIdx) => {
                    const isVerb = result.verbs.includes(wordIdx);
                    const wasClicked = result.clicked.includes(wordIdx);

                    if (isVerb && wasClicked) {
                      return (
          ...
          <button
            type="button"
            className="mt-6 min-h-14 rounded-2xl bg-teal-500 px-12 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-teal-600"
            onClick={restart}
          >
            Skúsiť znova
          </button>
