...
import { DATA_SLOVESO_KATEGORIE } from "@/src/data/sloveso-kategorie";
...
const casy = ["minulý", "prítomný", "budúci"];
...
type Question = (typeof DATA_SLOVESO_KATEGORIE)[number];
...
function pickQuestions(): Question[] {
  return shuffleArray(DATA_SLOVESO_KATEGORIE).slice(0, NUM_QUESTIONS);
}
...
function renderHighlightedSentence(sentence: string) {
  ...
  return (
    <>
      {match[1]}
      <span className="font-extrabold text-indigo-600 underline decoration-4">{match[2]}</span>
      {match[3]}
    </>
  );
}

export default function SlovesoKategoriePage() {
  const [questions, setQuestions] = useState<Question[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [selected, setSelected] = useState<Selected>({ osoba: null, cislo: null, cas: null });
  ...
  const setUpQuestion = (index: number, baseQuestions: Question[]) => {
    ...
    setSelected({ osoba: null, cislo: null, cas: null });
    setChecked(false);
    setFeedback("");
    setFeedbackTone("");
    setShowNext(false);
  ...
  const checkAnswer = () => {
    ...
    if (selected.osoba === null || selected.cislo === null || selected.cas === null) return;

    setChecked(true);

    const answer = currentQuestion.answer;
    const osobaOk = selected.osoba === answer.osoba;
    const cisloOk = selected.cislo === answer.cislo;
    const casOk = selected.cas === answer.cas;

    if (osobaOk && cisloOk && casOk) {
      setScore((prev) => prev + 1);
      setFeedback(`Správne! ${pickRandomMessage(PRAISE_MESSAGES)}`);
      setFeedbackTone("good");
    } else {
      const word = currentQuestion.sentence.match(/<(.+?)>/)?.[1] ?? "sloveso";
      const detail: Mistake["details"] = [];

      if (!osobaOk) {
        detail.push({
          cat: "osoba",
          yours: selected.osoba !== null ? osobaLabels[selected.osoba] : "-",
          correct: osobaLabels[answer.osoba],
        });
      }
      if (!cisloOk) {
      ...
      if (!casOk) {
        detail.push({ cat: "čas", yours: selected.cas, correct: answer.cas });
      }
  ...
  const isCheckDisabled =
    checked || selected.osoba === null || selected.cislo === null || selected.cas === null;

  if (!currentQuestion) return null;

  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-100 to-indigo-50 font-sans text-slate-800">
      {!showResults ? (
        <div>
            ...
            {questions.map((_, i) => (
              <div
                ...
                className={cn(
                  ...
                  i === currentIndex && "scale-125 bg-indigo-500",
                  i > currentIndex && "bg-indigo-200/80",
                )}
              />
            ))}
          </div>
          ...
          <main className="mx-auto flex min-h-[calc(100vh-7rem)] w-full max-w-5xl flex-col items-center justify-center px-4 py-8 sm:px-6">
            ...
            <p className="mb-7 text-center text-lg font-medium text-slate-500">
              Urči osobu, číslo a čas zvýrazneného slovesa
            </p>

            <div className="mb-6 w-full max-w-3xl space-y-4">
              <div className="rounded-2xl bg-white/95 p-4 shadow-sm">
                <p className="mb-3 text-lg font-extrabold uppercase tracking-wide text-indigo-600">Osoba</p>
                <div className="flex flex-wrap gap-2">
                  {osoby.map((option) => {
                    const isSelected = selected.osoba === option;
                    const isCorrect = checked && option === currentQuestion.answer.osoba;
                    const isWrong = checked && option === selected.osoba && selected.osoba !== currentQuestion.answer.osoba;
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          !checked && "border-indigo-200 bg-white hover:border-indigo-400 hover:bg-indigo-50",
                          !checked && isSelected && "border-indigo-500 bg-indigo-100 text-indigo-700",
                          isCorrect && "border-emerald-400 bg-emerald-100 text-emerald-700",
                          isWrong && "border-rose-300 bg-rose-100 text-rose-700",
                        )}
                        disabled={checked}
                        onClick={() => setSelected((prev) => ({ ...prev, osoba: option }))}
                      >
                        {osobaLabels[option]}
                      </button>
                    );
              ...
              <div className="rounded-2xl bg-white/95 p-4 shadow-sm">
                <p className="mb-3 text-lg font-extrabold uppercase tracking-wide text-indigo-600">Číslo</p>
                <div className="flex flex-wrap gap-2">
                  {cisla.map((option) => {
              ...
              <div className="rounded-2xl bg-white/95 p-4 shadow-sm">
                <p className="mb-3 text-lg font-extrabold uppercase tracking-wide text-indigo-600">Čas</p>
                <div className="flex flex-wrap gap-2">
                  {casy.map((option) => {
                    const isSelected = selected.cas === option;
                    const isCorrect = checked && option === currentQuestion.answer.cas;
                    const isWrong = checked && option === selected.cas && selected.cas !== currentQuestion.answer.cas;
                    ...
                    return (
                      <button
                        ...
                        disabled={checked}
                        onClick={() => setSelected((prev) => ({ ...prev, cas: option }))}
                      >
                        {option}
                      </button>
                    );
            ...
            {!checked && (
              <button
                type="button"
                className="min-h-14 rounded-2xl bg-indigo-500 px-10 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-indigo-600 disabled:cursor-default disabled:bg-indigo-300"
            ...
            {showNext && (
              <button
                type="button"
                className="mt-2 min-h-14 rounded-2xl bg-indigo-500 px-10 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-indigo-600"
        ...
        <section className="mx-auto w-full max-w-4xl px-4 py-12 text-center">
          <h2 className="text-4xl font-black text-indigo-600">Hotovo!</h2>
          ...
          <button
            type="button"
            className="mt-6 min-h-14 rounded-2xl bg-indigo-500 px-12 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-indigo-600"
            onClick={restart}
          >
            Skúsiť znova
          </button>
