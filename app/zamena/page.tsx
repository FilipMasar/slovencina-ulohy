...
import { DATA_ZAMENA } from "@/src/data/zamena";
import { cn } from "@/src/lib/cn";
import { shuffleArray } from "@/src/lib/shuffle";

const druhy = ["základné", "privlastňovacie"] as const;
...
function pickQuestions(): Question[] {
  return shuffleArray(DATA_ZAMENA).slice(0, NUM_QUESTIONS);
}

function getStarsAndMessage(pct: number) {
  if (pct >= 90) {
    return { stars: "⭐⭐⭐", message: "Si majster zámen!" };
  }
  if (pct >= 70) {
...
function renderHighlightedSentence(sentence: string) {
  ...
  return (
    <>
      {match[1]}
      <span className="font-extrabold text-rose-600 underline decoration-4">{match[2]}</span>
      {match[3]}
    </>
  );
}

export default function ZamenaPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-100 to-rose-50 font-sans text-slate-800">
      {!showResults ? (
        <div>
            ...
            {questions.map((_, i) => (
              <div
                ...
                className={cn(
                  ...
                  i === currentIndex && "scale-125 bg-rose-500",
                  i > currentIndex && "bg-rose-200/80",
                )}
              />
            ))}
          </div>
          ...
          <main className="mx-auto flex min-h-[calc(100vh-7rem)] w-full max-w-5xl flex-col items-center justify-center px-4 py-8 sm:px-6">
            ...
            <p className="mb-7 text-center text-lg font-medium text-slate-500">
              Aké zámeno je zvýraznené slovo?
            </p>

            <div className="mb-6 w-full max-w-3xl">
              <div className="rounded-2xl bg-white/95 p-4 shadow-sm">
                <p className="mb-3 text-lg font-extrabold uppercase tracking-wide text-rose-600">Druh</p>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          !checked && "border-rose-200 bg-white hover:border-rose-400 hover:bg-rose-50",
                          !checked && isSelected && "border-rose-500 bg-rose-100 text-rose-700",
            ...
            {!checked && (
              <button
                type="button"
                className="min-h-14 rounded-2xl bg-rose-500 px-10 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-rose-600 disabled:cursor-default disabled:bg-rose-300"
            ...
            {showNext && (
              <button
                type="button"
                className="mt-2 min-h-14 rounded-2xl bg-rose-500 px-10 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-rose-600"
        ...
        <section className="mx-auto w-full max-w-4xl px-4 py-12 text-center">
          <h2 className="text-4xl font-black text-rose-600">Hotovo!</h2>
          ...
          <button
            type="button"
            className="mt-6 min-h-14 rounded-2xl bg-rose-500 px-12 py-3 text-xl font-bold text-white transition hover:-translate-y-0.5 hover:bg-rose-600"
            onClick={restart}
          >
            Skúsiť znova
          </button>
